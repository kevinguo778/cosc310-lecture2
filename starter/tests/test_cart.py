...
def test_clear_empties_cart_and_resets_total() -> None:
    cart = Cart()
    cart.add_item(GYOZA, 2)
    cart.add_item(RAMEN)
    cart.clear()
...
def test_remove_item_removes_entire_quantity() -> None:
    cart = Cart()
...
def test_total_rounds_once_at_the_end() -> None:
    cart = Cart()
...
def test_negative_quantity_does_not_change_existing_cart() -> None:
    cart = Cart()
    cart.add_item(GYOZA, 2)
    with pytest.raises(ValueError):
        cart.add_item(GYOZA, -1)
...
def test_unavailable_item_does_not_change_existing_cart() -> None:
    cart = Cart()
    cart.add_item(GYOZA)
    with pytest.raises(OutOfStockError):
